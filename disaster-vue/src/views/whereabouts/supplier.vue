    ...
    <el-card class="box-card">
      ...
      <template>
        ...
        >
          ...
          <el-table-column label="操作" fixed="right" width="180" v-if="user.role === 'ROLE_ADMIN'">
            <template slot-scope="scope">
              <el-button
                  type="text"
                  size="mini"
...
export default {
  ...
  methods: {
    ...
    async update() {
      this.$refs.editRuleFormRef.validate(async valid => {
        if (!valid) {
          ...
          request.put("/supplier/update/" + this.editRuleForm.id, this.editRuleForm).then(res => {
            if (res) {
              this.$notify({
                title: "成功",
                message: "来源信息更新",
              ...
              this.$message.error("来源信息更新失败:" + res.data.msg);
            }
    ...
    //编辑
    async edit(id) {
      request.get("/supplier/edit/" + id).then(res => {
        console.log(res);
        if (res) {
          this.editRuleForm = res.data;
        } else {
          return this.$message.error("来源信息编辑失败" + res.data.msg);
        }
    ...
    add() {
      this.$refs.addRuleFormRef.validate(async valid => {
        if (!valid) {
          ...
          request.post("/supplier/",this.addRuleForm).then(res => {
            if (res) {
              this.$message.success("来源添加成功");
              this.addRuleForm = {};
              this.getSupplierList();
            } else {
              return this.$message.error("来源添加失败:" + res.data.msg);
            }
    ...
    async getSupplierList() {
      request.get("/supplier/findSupplierList",{
        params: this.queryMap
      }).then(res => {
        if (!res) {
          return this.$message.error("获取用户列表失败:"+res.data.msg);
        } else {
    ...
    _getJsonData() {
      request.get("/json/provinces.json").then(res => {
        this.provinceList = [];
        this.cityList = [];
        this.originList = [];
        res.forEach(item => {
