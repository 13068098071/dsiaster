...
export default {
  ...
  methods: {
    ...
    async del(id) {
      ...
      if (res === "confirm") {
        request.delete("/consumer/delete/"+id).then(res => {
          if (res.code === '200') {
    ...
    async update() {
      this.$refs.editRuleFormRef.validate(async valid => {
        if (!valid) {
          ...
          request.put("/consumer/update/"+this.editRuleForm.id,this.editRuleForm).then(res => {
            if (res) {
              this.$notify({
                title: "成功",
                message: "物资去处更新",
              ...
              this.$message.error("物资去处更新失败:" + res.data.msg);
            }
    ...
    async edit(id) {
      this._getJsonData();
      request.get("/consumer/edit/"+id).then(res => {
        if (res) {
          this.editRuleForm = res.data;
        } else {
          return this.$message.error("物资去处编辑失败" + res.data.msg);
        }
    ...
    add() {
      this.$refs.addRuleFormRef.validate(async valid => {
        if (!valid) {
          ...
          request.post("/consumer/add",this.addRuleForm).then(res => {
            if (res) {
              this.$message.success("物资去处添加成功");
              this.addRuleForm = {};
              this.getConsumerList();
            } else {
              return this.$message.error("物资去处添加失败:" + res.data.msg);
            }
    ...
    async getConsumerList() {
      request.get("/consumer/findConsumerList",{
