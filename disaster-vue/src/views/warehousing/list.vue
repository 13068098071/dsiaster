  ...
  <div id="inStocks">
    ...
    <el-card>
      ...
      <el-form size="small" :inline="true" :model="queryMap" class="demo-form-inline">
        ...
        <el-form-item>
          <el-button type="primary" @click="exportInStock">导出 <i class="el-icon-top"></i></el-button>
        </el-form-item>
        ...
        <el-table-column label="操作" fixed="right" width="200">
          <template slot-scope="scope">
            ...
            <span v-if="scope.row.status==2">
              <el-button type="text" @click="publish(scope.row.id)" v-if="user.role === 'ROLE_ADMIN'" slot="reference" size="small" icon="el-icon-refresh">通过</el-button>
...
export default {
  ...
  methods: {
    ...
    async loadTableData() {
      ...
      request.get("/in-stock/findInStockList",{
        params: this.queryMap
      }).then(res => {
        if (res.code != 200) {
          return this.$message.error("获取列表失败:"+res.msg);
        } else {
    ...
    async detail(id) {
      this.detailId=id;
      request.get("/in-stock/detail/" + id +"?pageNum="+this.pageNum).then(res => {
        if (res.code != 200) {
          this.$message.error("获取明细失败:" + res.msg);
        } else {
          this.detailTable = res.data.itemVOS;
          this.detailTotal = res.data.records;
     ...
     */
    async publish(id) {
      request.put("/in-stock/publish/" + id).then(res => {
        if (res.code != 200) {
     ...
     */
    async remove(id) {
      request.put("/in-stock/remove/"+id).then(res=>{
        if (res.code != 200) {
     ...
     */
    async back(id){
      request.put("/in-stock/back/"+id).then(res =>{
        if (res.code != 200) {
     ...
     */
    async del(id) {
      request.get("/in-stock/delete/"+id).then(res => {
        if (res.code != 200) {
     ...
     */
    async exportInStock(){
      window.open(`http://localhost:8889/in-stock/export`)
    }
  },
  created() {
